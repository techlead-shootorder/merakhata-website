...
class TriggersTableSeeder extends Seeder
{
    ...
    public $conditions = [
        ...
        'ticket:mailbox address' => [
            'type' => 'ticket:mailbox address',
            'name' => 'Ticket: Mailbox address',
            'operators' => [0, 1, 2, 3, 4, 5, 6],
        ],
        'customer:name' => [
            'type' => 'customer:name',
        ...
        // time based
        'timeframe:hours since created' => [
            'type' => 'timeframe:hours since created',
            'name' => 'Timeframe: Hours since created',
            'timeBased' => true,
            'operators' => [9, 7, 8],
        ],
        'timeframe:hours since closed' => [
            'type' => 'timeframe:hours since closed',
            'name' => 'Timeframe: Hours since closed',
            'timeBased' => true,
            'operators' => [9, 7, 8],
        ],
        'timeframe:hours since last activity' => [
            'type' => 'timeframe:hours since last activity',
            'name' => 'Timeframe: Hours since last activity',
            'timeBased' => true,
            'operators' => [9, 7, 8],
        ],
        'timeframe:hours since last reply' => [
            'type' => 'timeframe:hours since last reply',
            'name' => 'Timeframe: Hours since last reply',
            'timeBased' => true,
            'operators' => [9, 7, 8],
        ],
    ];
    ...
    private function seedActions()
    {
        ...
        $actions = array_map(function ($action) {
            $action['input_config'] = json_encode(
                $this->actionInputConfigs[$action['name']],
            );
            return $action;
        }, $this->actions);
        ...
        $this->condition->insert(
            array_map(function ($condition) {
                return [
                    'name' => $condition['name'],
                    'type' => $condition['type'],
                    'time_based' => $condition['timeBased'] ?? false,
                ];
            }, $this->conditions),
        );

        //generate insert for attaching operators to conditions in one query
        $pivot = $this->condition
            ->get()
            ->map(function ($condition) use ($operators) {
