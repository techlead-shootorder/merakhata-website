...
class TagRepository
{
    ...
    {
        $activeTickets = $this->ticket
            ->whereHas('tags', function (Builder $q) {
                $q->where('name', 'open');
            })
            ->select(['id', 'assigned_to'])
            ->with([
                'tags' => function (MorphToMany $q) {
                    $q->select('id');
                },
            ])
            ->limit(100)
        ...
        $categoryTags = $this->tag
            ...
            ->map(function (Tag $tag) use ($activeTickets) {
                $tag->tickets_count = $activeTickets
                    ->filter(function (Ticket $ticket) use ($tag) {
    ...
    public function attachById(Ticket $ticket, $tagIds)
    {
        ...
        $ticket->tags()->attach(array_diff($tagIds, $alreadyAttached));
    }
    ...
    {
        if ($taggables) {
            $taggableIds = array_map(function ($taggable) {
    ...
    {
        return $this->tag->updateOrCreate(
            ['name' => $data['name']],
            [
                'type' => isset($data['type']) ? $data['type'] : 'custom',
                'display_name' => isset($data['display_name'])
                    ? $data['display_name']
                    : $data['name'],
            ],
        );
