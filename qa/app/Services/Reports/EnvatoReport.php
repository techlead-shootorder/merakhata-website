...
class EnvatoReport
{
    ...
    public function getYearlyEarnings($params = [])
    {
        ...
        if ($this->cache->has($cacheKey)) {
            ...
            $response = $this->envatoClient->call(
                'market/private/user/earnings-and-sales-by-month.json',
                [],
                'v1',
            );
            $data = head($response);
    ...
    public function getMonthlyEarnings($params = [])
    {
        ...
        if (
            isset($dates['monthKey']) &&
            $this->cache->has($dates['monthKey'])
        ) {
            $response = $this->filterFormattedDataByRange(
                $this->cache->get($dates['monthKey']),
                $params,
            );
        }
        ...
        if (isset($params['envato_item_id'])) {
            return $this->filterFormattedDataByItem(
                $response,
                $params['envato_item_id'],
            );
        }
    ...
    private function fetchAndFormatEnvatoMonthlyEarnings($dates)
    {
        ...
        $r = $this->envatoClient->call('user/statement', [
            'from_date' => $dates['from'],
            'to_date' => $dates['to'],
        ]);
        $data = $r['results'];

        if (isset($r['pagination']['pages'])) {
            $pages = $r['pagination']['pages'];
            for ($i = 2; $i <= $pages; $i++) {
                // TODO: cache individual statement page calls based on params, previous months can be cached for a long time
                $r = $this->envatoClient->call('user/statement', [
                    'from_date' => $dates['from'],
                    'to_date' => $dates['to'],
                    'page' => $i,
        ...
        $response['totals'] = $this->calculateTotalsForMonth(
            $response['monthly'],
        );
        $response['items'] = $this->calculateItemSales(
            $formatted,
            $response['totals'],
        );
        ...
        if (!isset($dates['monthKey'])) {
            $cacheTime = $dates['isCurrentMonth']
                ? 60
    ...
    private function filterFormattedDataByItem($data, $itemId)
    {
        $data['sales'] = array_values(
            array_filter($data['sales'], function ($sale) use ($itemId) {
    ...
    private function calculateItemSales($data, $totals)
    {
        ...
        foreach ($data as $item) {
            ...
            if (isset($items[$item['item_id']])) {
                ...
                $items[$item['item_id']] = [
                    ...
                    'sales' => 1,
                    'name' => $item['item'],
                    'envato_id' => $item['item_id'],
                ];
            }

            $items[$item['item_id']]['amount'] = $this->formatFloat(
        ...
        foreach ($items as $k => $item) {
            $items[$k]['percentage'] = round(
                ($item['amount'] / $totals['earnings']) * 100,
            );
        }
     ...
     */
    private function calculateTotalsForMonth(array $data): array
    {
    ...
    private function formatDates($params = [])
    {
        ...
        $rangeKey =
            'envato.' . ((string) $from->timestamp . (string) $to->timestamp);

        //cache key for whole month (1st to 31st)
        $firstOfMonth = Carbon::createFromTimestamp(
            $from->timestamp,
        )->firstOfMonth();
        $lastOfMonth = Carbon::createFromTimestamp(
            $from->timestamp,
        )->lastOfMonth();
        $monthKey =
            'envato.' .
            ((string) $firstOfMonth->timestamp .
                (string) $lastOfMonth->timestamp);
    ...
    private function formatStatementData($data)
    {
        ...
        [$refunds, $sales] = collect($data)->partition(function ($item) {
        ...
        $formatted = $sales
            ...
            ->map(function (Collection $saleGroup) {
                [$fees, $sales] = $saleGroup->partition(function ($item) {
    ...
    private function generateMonthlyEarnings($sales)
    {
        ...
        foreach ($sales as $k => $item) {
            ...
            $monthly[$item['day']]['amount'] = $this->formatFloat(
                $monthly[$item['day']]['amount'],
            );
        }
