    ...
    public function execute(string $path)
    {
        ...
        collect($helpCenter)->each(function ($parentData) use ($oldBaseUri) {
            ...
            $parent = $this->category->create(
                Arr::except($parentData, ['children', 'id', 'model_type']),
            );

            //create child categories
            collect($parentData['children'])->each(function ($childData) use (
                $parent,
                $oldBaseUri
            ) {
                $childData['parent_id'] = $parent->id;
                $child = $this->category->create(
                    Arr::except($childData, ['articles', 'id', 'model_type']),
                );

                //create articles
                collect($childData['articles'])->each(function (
                    $articleData
                ) use ($child, $parent, $oldBaseUri) {
                    $articleData['body'] = app(
                        ReplaceArticleImageSrc::class,
                    )->execute($articleData['body'], $oldBaseUri);

                    $article = $this->article->firstOrCreate(
                        Arr::except($articleData, ['tags', 'id', 'model_type']),
                    );
