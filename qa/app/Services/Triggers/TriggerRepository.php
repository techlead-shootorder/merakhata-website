...
class TriggerRepository
{
    ...
    public function findOrFail(int $id): Trigger
    {
        $trigger = $this->trigger
            ->with(['conditions.operators', 'actions'])
            ->findOrFail($id);
        return $this->attachSelectedOperator([$trigger])[0];
    }
    ...
    public function create($params)
    {
        ...
        $conditionInsert = array_map(
            'unserialize',
            array_unique(array_map('serialize', $conditionInsert)),
        );
        $trigger
            ->conditions()
            ->newPivotStatement()
        ...
        $actionInsert = array_map(
            'unserialize',
            array_unique(array_map('serialize', $actionInsert)),
        );
        $trigger
            ->actions()
            ->newPivotStatement()
    ...
    /**
     * @param Collection|array $triggers
    ...
    private function attachSelectedOperator($triggers)
    {
        ...
        foreach ($triggers as $trigger) {
            $operatorsIds = $operatorsIds->merge(
                $trigger->conditions->map(function ($condition) {
