        ...
        return Cache::remember(
            ...
            function () {
                ...
                $categories = $this->category
                    ...
                    ->with($loadArticles)
                    ->with([
                        'children' => function (HasMany $query) use (
                            $loadArticles
                        ) {
                            $query
                                ->where('hidden', false)
                ...
                $categoryLimit = $this->settings->get(
                    'hc_home.children_per_category',
                    6,
                );
                $articleLimit = $this->settings->get(
                    'hc_home.articles_per_category',
                    5,
                );

                return $categories->each(function (Category $category) use (
                    $categoryLimit,
                    $articleLimit
                ) {
                    ...
                    $category->setRelation(
                        'children',
                        $category->children->take($categoryLimit),
                    );
                    $category->children->each(function (Category $child) use (
                        $articleLimit
                    ) {
                        $child->setRelation(
                            'articles',
                    ...
                    if ($category->children->isEmpty()) {
                        $category->chunked_articles = $category->articles
                            ->values()
                            ->chunk($articleLimit)
                            ->map->values()
                            ->take($categoryLimit);
